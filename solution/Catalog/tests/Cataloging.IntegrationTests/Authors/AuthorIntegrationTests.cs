...
public sealed class AuthorIntegrationTests : IntegrationContext
{
    ...
    public AuthorIntegrationTests(AppFixture app, ITestOutputHelper output) : base(app)
    {
        _app = app;
        _output = output;
    }
    ...
    [Fact]
    public async Task Get_Top3_Returns3Authors_Wolverine()
    {
        // Act
        var (tracked, response) = await TrackedHttpCall(scenario =>
        {
            scenario.Get.Url("/v1/authors?$top=3");
        });
